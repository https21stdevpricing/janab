...
export function PrintDoc({ kind, id }: { kind: "invoice" | "quote"; id: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: h } = (await supabase.from(table).select("*").eq("id", id).maybeSingle()) as {
        data: any;
      };
      setDoc(h);
      if (h) {
  ...
  const totals = useMemo(() => {
    ...
    const gst = items.reduce(
      (a, it) => a + (Number(it.qty || 0) * Number(it.rate || 0) * Number(it.gst_pct ?? 0)) / 100,
      0,
    );
  ...
  const sameState = !!(
    company?.state &&
    buyer?.state &&
    company.state.toLowerCase() === buyer.state.toLowerCase()
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const payload =
        design.qrMode === "upi-pay" && upiPayload ? upiPayload : digitalCopyUrl(documentNo);
  ...
  useEffect(() => {
    if (design.barcodeMode !== "auto" || !documentNo) {
      setAutoBarcode(null);
      return;
    }
  ...
  const uploadLogo = async (file?: File) => {
    if (file) updateDesign({ ...design, logoDataUrl: await fileToDataUrl(file) });
  };
  const uploadWatermarkLogo = async (file?: File) => {
    if (file) updateDesign({ ...design, watermarkLogoDataUrl: await fileToDataUrl(file) });
  };
  const uploadQr = async (file?: File) => {
    if (file) updateDesign({ ...design, qrCodeDataUrl: await fileToDataUrl(file) });
  };
  const uploadBarcode = async (file?: File) => {
    if (file) updateDesign({ ...design, barcodeDataUrl: await fileToDataUrl(file) });
  };
  const uploadFooterLogos = async (files?: FileList | null) => {
    if (!files) return;
    const add = await Promise.all(
      Array.from(files)
        .slice(0, 20 - design.footerLogos.length)
        .map(fileToDataUrl),
    );
          ...
          <summary className="flex cursor-pointer items-center justify-between gap-3 list-none [&::-webkit-details-marker]:hidden">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  updateDesign(DEFAULT_PRINT_DESIGN);
                }}
              >
                Reset
              </Button>
              <span className="rounded-full border bg-background px-2.5 py-1 text-[10px] uppercase tracking-wide text-muted-foreground group-open:bg-primary/10 group-open:text-primary">
                Customize
              </span>
            </div>
          </summary>
          <div className="mt-3 space-y-2.5">
            <div className="grid gap-2 sm:grid-cols-3">
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  value={design.preset}
                  onChange={(e) => updatePreset(e.target.value as PrintPreset)}
                >
                  {(Object.keys(presetLabels) as PrintPreset[]).map((key) => (
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) => updateDesign({ ...design, headerStyle: e.target.value as any })}
                >
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  className="h-8 rounded-md border bg-background px-2 text-xs"
                  value={design.bodyLayout}
                  onChange={(e) => updateDesign({ ...design, bodyLayout: e.target.value as any })}
                >
              ...
              <label className="flex flex-col gap-1">
                ...
                <span className="h-8 rounded-md border bg-background px-2 text-xs flex items-center justify-between gap-2 cursor-pointer">
                  <span className="truncate">
                    {design.logoDataUrl ? "Custom uploaded" : "Default StoneWorld"}
                  </span>
            ...
            <div className="rounded-md border border-border/60 p-2 space-y-2">
              ...
              <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                <input
                  className="h-8 rounded-md border bg-background px-2 text-xs"
                  placeholder="Watermark text (e.g. DRAFT, PAID)"
                  value={design.watermarkText ?? ""}
                  onChange={(e) => updateDesign({ ...design, watermarkText: e.target.value })}
                />
                <label className="flex items-center gap-2 text-[11px]">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateDesign({ ...design, watermarkOpacity: Number(e.target.value) })
                    }
                    className="w-24"
                  />
                ...
                <select
                  className="h-8 rounded-md border bg-background px-2 text-xs"
                  value={design.watermarkLayer}
                  onChange={(e) =>
                    updateDesign({ ...design, watermarkLayer: e.target.value as any })
                  }
                >
                  <option value="back">Behind content</option>
                  <option value="front">Over content</option>
                </select>
              </div>
              <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                <label className="h-8 rounded-md border bg-background px-2 text-xs flex items-center justify-between gap-2 cursor-pointer">
                  <span className="truncate">
                    {design.watermarkLogoDataUrl
                      ? "Logo watermark uploaded"
                      : "Logo watermark (PNG)"}
                  </span>
                  <span className="text-primary">
                    {design.watermarkLogoDataUrl ? "Replace" : "Upload"}
                  </span>
                  <input
                    type="file"
                    accept="image/png,image/jpeg"
                    className="hidden"
                    onChange={(e) => uploadWatermarkLogo(e.target.files?.[0])}
                  />
                </label>
                <label className="flex items-center gap-2 text-[11px]">
                  <span>Size</span>
                  <input
                    ...
                    min={20}
                    max={90}
                    value={design.watermarkLogoScale}
                    onChange={(e) =>
                      updateDesign({ ...design, watermarkLogoScale: Number(e.target.value) })
                    }
                    className="w-24"
                  />
                ...
                {design.watermarkLogoDataUrl && (
                  <Button
                    ...
                    size="sm"
                    className="h-8 px-2"
                    onClick={() => updateDesign({ ...design, watermarkLogoDataUrl: null })}
                  >
                    Remove logo
                  </Button>
                )}
            ...
            <div className="rounded-md border border-border/60 p-2 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                  Footer brand logos ({design.footerLogos.length}/20)
                </div>
                {design.footerLogos.length > 0 && (
                  <Button
                    ...
                    size="sm"
                    className="h-7 px-2"
                    onClick={() => updateDesign({ ...design, footerLogos: [] })}
                  >
                    Clear
                  </Button>
                )}
              ...
              <div className="grid gap-2 sm:grid-cols-[auto_auto_auto_1fr]">
                <label className="h-8 rounded-md border bg-background px-3 text-[11px] flex items-center cursor-pointer">
                  Add PNG logos
                  <input
                    type="file"
                    accept="image/png"
                    multiple
                    className="hidden"
                    onChange={(e) => uploadFooterLogos(e.target.files)}
                  />
                </label>
                <label className="flex items-center gap-2 text-[11px]">
                  <span>Rows</span>
                  <select
                    className="h-8 rounded-md border bg-background px-2 text-xs"
                    value={design.footerRows}
                    onChange={(e) =>
                      updateDesign({ ...design, footerRows: Number(e.target.value) as 1 | 2 | 3 })
                    }
                  >
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateDesign({ ...design, footerLogoSize: Number(e.target.value) })
                    }
                    className="w-24"
                  />
                  <span className="tabular-nums w-10 text-right">{design.footerLogoSize}px</span>
                </label>
                <label className="flex items-center gap-2 text-[11px] sm:justify-end">
                  <span>Place at</span>
                  <select
                    className="h-8 rounded-md border bg-background px-2 text-xs"
                    value={design.footerPosition}
                    onChange={(e) =>
                      updateDesign({ ...design, footerPosition: e.target.value as any })
                    }
                  >
                    <option value="above-signature">Above signature</option>
                    <option value="page-bottom">At page bottom</option>
                  </select>
                </label>
              </div>
              <label className="flex items-center gap-2 text-[11px]">
                <input
                  type="checkbox"
                  checked={design.footerOnEveryPage}
                  onChange={(e) => updateDesign({ ...design, footerOnEveryPage: e.target.checked })}
                />
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {design.footerLogos.map((src, i) => (
                    <span key={i} className="relative rounded border bg-background p-1">
                      ...
                      <button
                        type="button"
                        className="absolute -top-1.5 -right-1.5 h-4 w-4 grid place-items-center rounded-full bg-destructive text-destructive-foreground text-[9px]"
                        onClick={() =>
                          updateDesign({
                            ...design,
                            footerLogos: design.footerLogos.filter((_, j) => j !== i),
                          })
                        }
                        aria-label="Remove"
                      >
            ...
            <div className="rounded-md border border-border/60 p-2 space-y-2">
              <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                Document extras (pre-filled blocks)
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    ...
                    onChange={(e) => updateDesign({ ...design, showBankDetails: e.target.checked })}
                  />
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    checked={design.showUpi}
                    onChange={(e) => updateDesign({ ...design, showUpi: e.target.checked })}
                  />
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    checked={design.showGstSummary}
                    onChange={(e) => updateDesign({ ...design, showGstSummary: e.target.checked })}
                  />
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    checked={design.showHsnSummary}
                    onChange={(e) => updateDesign({ ...design, showHsnSummary: e.target.checked })}
                  />
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    checked={design.showTaxInWords}
                    onChange={(e) => updateDesign({ ...design, showTaxInWords: e.target.checked })}
                  />
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    checked={design.showShipTo}
                    onChange={(e) => updateDesign({ ...design, showShipTo: e.target.checked })}
                  />
                ...
                <label className="flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    checked={design.showTransport}
                    onChange={(e) => updateDesign({ ...design, showTransport: e.target.checked })}
                  />
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    className="h-8 rounded-md border bg-background px-2 text-xs"
                    value={design.roundOff}
                    onChange={(e) => updateDesign({ ...design, roundOff: e.target.value as any })}
                  >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    className="h-8 rounded-md border bg-background px-2 text-xs"
                    value={design.qrMode}
                    onChange={(e) => updateDesign({ ...design, qrMode: e.target.value as any })}
                  >
                    <option value="digital-copy">Auto · Digital copy link</option>
                    <option value="upi-pay" disabled={!company?.upi_id}>
                      Auto · UPI scan &amp; pay{company?.upi_id ? "" : " (set UPI ID in Settings)"}
                    </option>
                  ...
                  {design.qrMode === "manual" && (
                    <label className="h-8 rounded-md border bg-background px-2 text-xs flex items-center justify-between gap-2 cursor-pointer">
                      <span className="truncate">
                        {design.qrCodeDataUrl ? "QR uploaded" : "Upload QR image"}
                      </span>
                      <span className="text-primary">
                        {design.qrCodeDataUrl ? "Replace" : "Upload"}
                      </span>
                      <input
                        type="file"
                        accept="image/png,image/jpeg"
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateDesign({ ...design, qrPlacement: e.target.value as PrintCodePlacement })
                    }
                    disabled={design.qrMode === "off"}
                  >
                    {(Object.keys(codePlacementLabels) as PrintCodePlacement[]).map((key) => (
                      <option key={key} value={key}>
                        {codePlacementLabels[key]}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    className="h-8 rounded-md border bg-background px-2 text-xs"
                    value={design.barcodeMode}
                    onChange={(e) =>
                      updateDesign({ ...design, barcodeMode: e.target.value as any })
                    }
                  >
                  ...
                  {design.barcodeMode === "manual" && (
                    <label className="h-8 rounded-md border bg-background px-2 text-xs flex items-center justify-between gap-2 cursor-pointer">
                      <span className="truncate">
                        {design.barcodeDataUrl ? "Barcode uploaded" : "Upload barcode image"}
                      </span>
                      <span className="text-primary">
                        {design.barcodeDataUrl ? "Replace" : "Upload"}
                      </span>
                      <input
                        type="file"
                        accept="image/png,image/jpeg"
                        className="hidden"
                        onChange={(e) => uploadBarcode(e.target.files?.[0])}
                      />
                    </label>
                  )}
                  <select
                    ...
                    onChange={(e) =>
                      updateDesign({
                        ...design,
                        barcodePlacement: e.target.value as PrintCodePlacement,
                      })
                    }
                    disabled={design.barcodeMode === "off"}
                  >
                    {(Object.keys(codePlacementLabels) as PrintCodePlacement[]).map((key) => (
                      <option key={key} value={key}>
                        {codePlacementLabels[key]}
                ...
                <label className="sm:col-span-2 flex items-center gap-2 text-[11px]">
                  ...
                  <input
                    ...
                    value={design.signatoryName ?? ""}
                    onChange={(e) => updateDesign({ ...design, signatoryName: e.target.value })}
                  />
                </label>
              </div>
                ...
                <div className="grid gap-2 sm:grid-cols-2 pt-1 border-t border-border/40 mt-1">
                  {design.showShipTo && (
                    <label className="sm:col-span-2 flex flex-col gap-1">
                      <span className="text-[10px] uppercase tracking-wide text-muted-foreground">
                        Ship-To address (leave blank to mirror Bill-To)
                      </span>
                      <textarea
                        rows={2}
                        className="rounded-md border bg-background px-2 py-1.5 text-xs"
                        placeholder="Consignee name, address, GSTIN…"
                        value={design.shipToOverride ?? ""}
                        onChange={(e) =>
                          updateDesign({ ...design, shipToOverride: e.target.value })
                        }
                      />
                    </label>
                  )}
                  {design.showTransport && (
                    <>
                      <label className="flex flex-col gap-1">
                        ...
                        <input
                          className="h-8 rounded-md border bg-background px-2 text-xs"
                          value={design.dispatchDocNo ?? ""}
                          onChange={(e) =>
                            updateDesign({ ...design, dispatchDocNo: e.target.value })
                          }
                        />
                      </label>
                      <label className="flex flex-col gap-1">
                        ...
                        <input
                          ...
                          value={design.transporter ?? ""}
                          onChange={(e) => updateDesign({ ...design, transporter: e.target.value })}
                        />
                      </label>
                      <label className="flex flex-col gap-1">
                        <span className="text-[10px] uppercase tracking-wide text-muted-foreground">
                        ...
                        <input
                          className="h-8 rounded-md border bg-background px-2 text-xs"
                          value={design.vehicleNo ?? ""}
                          onChange={(e) => updateDesign({ ...design, vehicleNo: e.target.value })}
                        />
                      </label>
                      <label className="flex flex-col gap-1">
                        <span className="text-[10px] uppercase tracking-wide text-muted-foreground">
                        ...
                        <input
                          className="h-8 rounded-md border bg-background px-2 text-xs"
                          value={design.destination ?? ""}
                          onChange={(e) => updateDesign({ ...design, destination: e.target.value })}
                        />
                      </label>
                    </>
                  )}
                  <label className="sm:col-span-2 flex flex-col gap-1">
                    <span className="text-[10px] uppercase tracking-wide text-muted-foreground">
                      Declaration / Terms (override)
                    </span>
                    <textarea
                      rows={2}
                      className="rounded-md border bg-background px-2 py-1.5 text-xs"
                      placeholder="Leave blank to use default T&C…"
                      value={design.declaration ?? ""}
                      onChange={(e) => updateDesign({ ...design, declaration: e.target.value })}
                    />
                  </label>
                </div>
              )}
      ...
      <div className="overflow-x-auto -mx-3 px-3 print:overflow-visible print:mx-0 print:px-0">
        ...
        >
          {(design.watermarkText || design.watermarkLogoDataUrl) && (
            <div
              ...
              style={{
                ...
                zIndex: design.watermarkLayer === "front" ? 30 : 0,
              }}
            >
              {design.watermarkLogoDataUrl && (
                <img
                  src={design.watermarkLogoDataUrl}
                  alt=""
                  style={{ width: `${design.watermarkLogoScale}%`, transform: "rotate(-8deg)" }}
                  className="object-contain"
                />
              )}
              {design.watermarkText && (
                <div
                  className="absolute text-6xl font-black uppercase tracking-tight"
                  style={{ color: "#0f172a", transform: "rotate(-24deg)" }}
                >
                  {design.watermarkText}
                </div>
              )}
          ...
          >
            ...
            {design.headerContainer === "open" ? null : <div className="mt-5 h-px bg-slate-200" />}
          </header>

          <section
            className={`px-9 py-6 grid gap-10 ${design.showShipTo ? "md:grid-cols-3" : "grid-cols-2"}`}
          >
            <InfoPanel
              title={kind === "invoice" ? "Bill To (Buyer)" : "Quoted To"}
              rows={[
            ...
            <InfoPanel
              title={kind === "invoice" ? "Invoice Info" : "Quotation Info"}
              rows={[
                [kind === "invoice" ? "Invoice No" : "Quote No", documentNo],
                ["Date", fmtDate(doc.date)],
                ...(doc.valid_until
                  ? [["Valid Until", fmtDate(doc.valid_until)] as [string, string]]
                  : []),
            ...
            (design.dispatchDocNo ||
              ...
              <section className="px-9 pb-2">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border border-slate-200 rounded p-3 text-[11px]">
                  {design.dispatchDocNo && (
                    <div>
                      <div className="text-[9.5px] uppercase tracking-wide text-[#6e7886]">
                        Dispatch Doc No.
                      </div>
                      <div className="font-semibold text-[#111621] mt-0.5">
                        {design.dispatchDocNo}
                      </div>
                    </div>
                  )}
                  ...
                  {design.vehicleNo && (
                    <div>
                      ...
                      <div className="font-semibold text-[#111621] mt-0.5 font-mono">
                        {design.vehicleNo}
                      </div>
                    </div>
                  )}
          ...
          >
            ...
            <div className="mt-8 grid grid-cols-[1fr_260px] gap-10 items-start break-inside-avoid">
              <div className="text-[11px] leading-5 text-[#374050]">
                <p className="text-[9.5px] font-bold uppercase tracking-[0.1em] text-[#6e7886]">
                  Amount in words
                </p>
                  ...
                  (company?.bank_name || company?.bank_account_no || company?.bank_ifsc) && (
                    <div className="mt-4">
                      ...
                      <div className="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-[11px]">
                        {company?.bank_name && (
                          <>
                            <span className="text-[#6e7886]">Bank</span>
                            <span className="font-semibold">{company.bank_name}</span>
                          </>
                        )}
                        {company?.bank_account_no && (
                          <>
                            <span className="text-[#6e7886]">A/c No.</span>
                        ...
                        {company?.bank_ifsc && (
                          <>
                            <span className="text-[#6e7886]">IFSC</span>
                            <span className="font-mono font-semibold">{company.bank_ifsc}</span>
                          </>
                        )}
                        {design.showUpi && company?.upi_id && (
                          <>
                            <span className="text-[#6e7886]">UPI</span>
                            <span className="font-mono font-semibold">{company.upi_id}</span>
                          </>
                        )}
                      </div>
                    </div>
                ...
                {qrPlacement === "terms" && renderedQr && (
                  <div className="mt-4 inline-flex flex-col items-start">
                    ...
                    {qrCaption && (
                      <span className="text-[9px] text-[#6e7886] mt-1">{qrCaption}</span>
                    )}
                  </div>
                )}
                {doc.notes && (
                  <p className="mt-4">
                    <span className="text-[9.5px] font-bold uppercase tracking-[0.1em] text-[#6e7886]">
                      Notes
                    </span>
                    <br />
                    {doc.notes}
                ...
                <p className="mt-4 text-[9.5px] font-bold uppercase tracking-[0.1em] text-[#6e7886]">
                  {kind === "invoice" ? "Terms & Conditions" : "Terms of Proposal"}
                </p>
                <p className="mt-1 text-[10.5px] leading-4 text-[#6e7886]">
                  {kind === "invoice"
                    ? "Goods once sold will not be taken back. Interest @18% p.a. on overdue balances. Subject to local jurisdiction. E&OE."
                    : "Prices valid until the date shown above. Quotation does not constitute a tax invoice. Stock and lot variation may apply. E&OE."}
                </p>
                {design.declaration && design.declaration.trim() && (
                  <p className="mt-1 text-[10.5px] leading-4 text-[#374050] whitespace-pre-line">
                    {design.declaration}
                  </p>
                )}
              </div>
              <div className="text-[11.5px] relative">
                ...
                {design.showGstSummary &&
                  (sameState ? (
                    <>
                      <SummaryLine label="CGST" value={totals.gst / 2} />
                      <SummaryLine label="SGST" value={totals.gst / 2} />
                    </>
                  ) : (
                    <SummaryLine label="IGST" value={totals.gst} />
                  ))}
            ...
            {kind === "invoice" && design.showHsnSummary && hsnSummary.length > 0 && (
              <div className="mt-6 break-inside-avoid">
                <p className="text-[9.5px] font-bold uppercase tracking-[0.1em] text-[#6e7886] mb-2">
                  HSN/SAC tax summary
                </p>
                <table className="w-full border-collapse text-[10.5px] leading-4 border border-slate-200">
                  <thead className="bg-slate-50 text-[#6e7886] uppercase text-[9px] tracking-[0.08em]">
                    <tr>
                      <th className="text-left py-1.5 px-2 border-b border-slate-200 font-bold">
                        HSN/SAC
                      </th>
                      <th className="text-right py-1.5 px-2 border-b border-slate-200 font-bold">
                        Taxable Value
                      </th>
                      {sameState ? (
                        <>
                          <th
                            className="text-center py-1.5 px-2 border-b border-slate-200 font-bold"
                            colSpan={2}
                          >
                            CGST
                          </th>
                    ...
                    <tr className="text-[9px]">
                      ...
                      {sameState ? (
                        <>
                          <th className="text-center py-1 px-2 border-b border-slate-200">Rate</th>
                          <th className="text-right py-1 px-2 border-b border-slate-200">Amount</th>
                          <th className="text-center py-1 px-2 border-b border-slate-200">Rate</th>
                          <th className="text-right py-1 px-2 border-b border-slate-200">Amount</th>
                        </>
                      ) : (
                ...
                {design.showTaxInWords && totals.gst > 0 && (
                  <p className="mt-2 text-[10.5px] text-[#374050]">
                    <span className="text-[9.5px] font-bold uppercase tracking-[0.1em] text-[#6e7886]">
                      Tax amount in words:{" "}
                    </span>
                    <span className="font-semibold text-[#111621]">
                      {amountInWords(totals.gst)}
                    </span>
                  </p>
                )}
              </div>
            ...
            {design.footerPosition === "above-signature" &&
              !design.footerOnEveryPage &&
              footerLogoBlock}
            <div className="mt-14 grid grid-cols-2 text-[11px] text-[#6e7886] break-inside-avoid">
              <div>Thank you for your business.</div>
              <div className="text-right">
                For {company?.company_name ?? "StoneWorld Traders"}
                <br />
                <br />
                <br />
                <span className="border-t border-slate-400 pt-2 inline-block min-w-[180px] font-semibold text-[#111621]">
                  {design.signatoryName?.trim() || "Authorised Signatory"}
                </span>
              </div>
            </div>
            {(design.footerPosition === "page-bottom" || design.footerOnEveryPage) &&
              footerLogoBlock}
          </section>
        </article>
      </div>
...
function ProductPrintTable({
  ...
  const qtyText = (value: unknown) =>
    fmt(Number(value ?? 0), Math.abs(Number(value ?? 0) % 1) < 0.0001 ? 0 : 2);
  const qtyFont = (value: string) =>
    value.length > 9 ? "text-[9px]" : value.length > 7 ? "text-[10px]" : "text-[11px]";
  return (
        ...
        {items.map((it, i) => {
          ...
          return (
            <tr key={i} className="border-b border-slate-100 break-inside-avoid">
              ...
              <td
                className={`${td} text-right tabular-nums font-mono whitespace-nowrap ${qtyFont(qty)}`}
              >
                {qty}
              </td>
...
function InfoPanel({ title, rows }: { title: string; rows: Array<[string, any]> }) {
  return (
    <div className="border-t border-slate-300 pt-3">
      <div className="text-[9.5px] font-bold uppercase tracking-[0.14em] text-[#6e7886] mb-3">
        {title}
      </div>
...
function SummaryLine({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex justify-between py-1.5">
      <span className="text-[#6e7886]">{label}</span>
      <span className="tabular-nums font-semibold text-[#111621]">{inr(value)}</span>
    </div>
  );
}
